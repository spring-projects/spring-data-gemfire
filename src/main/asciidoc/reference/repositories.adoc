...
{data-store-name} along with running queries. A general introduction to the Repository programming model
...
of a Repository query method and the property value is the OQL query you want to run when the Repository query method
is called.
...
[[gemfire-repositories.queries.running]]
== Running OQL Queries

{sdg-name} Repositories enable the definition of query methods to easily run {data-store-name} OQL queries
