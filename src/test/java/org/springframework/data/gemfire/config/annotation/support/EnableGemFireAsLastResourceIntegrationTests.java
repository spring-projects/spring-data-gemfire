 ...
 * @see com.gemstone.gemfire.ra.GFConnectionFactory
...
public class EnableGemFireAsLastResourceIntegrationTests {
	...
	private static List<SpringGemFireTransactionEvents> transactionEvents =
		new ArrayList<SpringGemFireTransactionEvents>();
	...
	static class TestGemFireAsLastResourceConfiguration {
		...
		GemFireCache mockGemFireCache() {
			...
			final AtomicBoolean copyOnRead = new AtomicBoolean(false);
			...
			doAnswer(new Answer<Void>() {
				...
				public Void answer(InvocationOnMock invocation) throws Throwable {
					copyOnRead.set(invocation.getArgumentAt(0, Boolean.class));
					return null;
				}
			}).when(mockGemFireCache).setCopyOnRead(anyBoolean());

			when(mockGemFireCache.getCopyOnRead()).thenAnswer(new Answer<Boolean>() {
				@Override
				public Boolean answer(InvocationOnMock invocationOnMock) throws Throwable {
					return copyOnRead.get();
				}
			});
		...
		GFConnectionFactory mockGemFireConnectionFactory() throws ResourceException {
			...
			final GFConnection mockGemFireConnection = mock(GFConnection.class);

			when(mockGemFireConnectionFactory.getConnection()).thenAnswer(new Answer<GFConnection>() {
				@Override
				public GFConnection answer(InvocationOnMock invocationOnMock) throws Throwable {
			...
			doAnswer(new Answer<Void>() {
				@Override public Void answer(InvocationOnMock invocationOnMock) throws Throwable {
					transactionEvents.add(SpringGemFireTransactionEvents.CLOSE_CONNECTION);
					return null;
				}
		...
		PlatformTransactionManager mockTransactionManager() {
			...
			when(mockTransactionManager.getTransaction(any(TransactionDefinition.class))).thenAnswer(
				new Answer<TransactionStatus>() {
					@Override
					public TransactionStatus answer(InvocationOnMock invocationOnMock) throws Throwable {
			...
			doAnswer(new Answer<Void>() {
				@Override public Void answer(InvocationOnMock invocationOnMock) throws Throwable {
					transactionEvents.add(SpringGemFireTransactionEvents.COMMIT);
					return null;
				}
			...
			doAnswer(new Answer<Void>() {
				@Override public Void answer(InvocationOnMock invocationOnMock) throws Throwable {
					transactionEvents.add(SpringGemFireTransactionEvents.ROLLBACK);
					return null;
				}
